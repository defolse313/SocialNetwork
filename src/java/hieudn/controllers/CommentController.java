...
import static hieudn.enitties.Comment_.articleId;
import hieudn.enitties.Member1;
import static hieudn.enitties.Member1_.memberId;
...
public class CommentController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
                    String url = ERROR;
        try {
            System.out.println("commentttttttttttttttt");
            String cmt = request.getParameter("txtComment");
            System.out.println("cmt:" + cmt);
            HttpSession session = request.getSession();
            Member1 mem = new Member1((String) session.getAttribute("ID"));
            int articleID = Integer.parseInt(request.getParameter("txtArticle"));
            System.out.println("articleId: " + articleId);
            java.util.Date d = new java.util.Date();
            Date d1 = new Date(d.getTime());
//            Comment c = new Comment(cmt, article, mem, d1);
            RgistrationBLO blo = new RgistrationBLO();
            boolean check = blo.cmt(articleID, mem.getMemberId(), cmt, d1);
            if(!check) {
                url = ERROR;
            } else {
                url = SUCCESS;
            ...
            log("Error at cmtController: " + e.getMessage());
            e.printStackTrace();
        } finally {
